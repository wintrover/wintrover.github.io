...
- **흐름 블록 가독성 규칙**: `요구사항 입력` 같은 흐름 블록은 빈 줄과 줄 끝 `\`를 포함한 단계형 레이아웃을 유지하고, 화살표(`→`) 연결 구문은 원문 배치(단일행/다중행)를 보존한다.
- **정형 검증 절차 표기 규칙**: 검증 절차를 설명하는 단락은 번호 목록(`1.`~`N.`)을 기본 형식으로 유지하고, 예시 문장은 조건문/결과문 종결부의 쉼표·마침표·역슬래시를 원문대로 보존한다.
