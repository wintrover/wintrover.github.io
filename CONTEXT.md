...
- 전역 규칙에 영향이 있는 요구사항은 먼저 이 문서에 반영한다.
- 기능 변경은 BDD(Given-When-Then) 시나리오를 `.feature` 파일로 먼저 정의한다.
- 이후 TDD로 실패 테스트를 작성하고, 테스트를 통과하는 최소 변경만 반영한다.
- 최종 검증에서는 테스트 통과 후 `scripts/build-github.ts` 실행 결과가 이 문서와 일치해야 한다.
