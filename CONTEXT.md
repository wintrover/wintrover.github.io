...
- [REQ-DEPLOY-17] 배포 후보 선정 시 `post_key`는 파일명(`basename`)만 사용하며 전체 경로를 포함하지 않아야 한다. `scripts/post-to-dev.ts`의 `toPostKey()` 함수는 반드시 `basename`만 반환해야 하며, 이를 위반할 경우 `[REQ-DEPLOY-17]` 메시지와 함께 에러를 발생시켜야 한다.
